    ...
    def __init__(
        ...
        if _weight is None:
            ...
            self.weight_leafs = nn.Parameter(
                torch.Tensor(  # TODO:  During backward do we need all of these parameters to be loaded ? can we do better?
                    self.order, self.rank, self.num_embeddings, self.embedding_dim_leaf
                )
            )
            logging.info(
                "EmbeddingKet weight_leafs shape: " + str(self.weight_leafs.shape)
            )
...
class EmbeddingKetXS(nn.Embedding):
    ...
    def __init__(
        ...
        self.embedding_dim_leaf = math.ceil((embedding_dim) ** (1 / order))
        logging.info(
            "EmbeddingKetXS num_embeddings_leaf: " + str(self.num_embeddings_leaf)
        )
        logging.info(
            "EmbeddingKetXS embedding_dim_leaf: " + str(self.embedding_dim_leaf)
        )
        self.rank = rank
        if padding_idx is not None:
            if padding_idx > 0:
        ...
        if _weight is None:
            ...
            logging.info(
                "EmbeddingKetXS weight_leafs shape: " + str(self.weight_leafs.shape)
            )
            self.reset_parameters()
        else:
            assert list(_weight.shape) == [
    ...
    def forward(self, input):
        ...
        if self.lazy:
            self.weight = KroneckerProductLazyTensor(*NonLazyTensor(w)).sum(
                dim=0
            )  # get the sum of the batch of product
            logging.debug("self.weight.shape: " + str(self.weight.shape))
            if input.dim() == 1:  #
                return (
                    self.weight[input]
                    .base_lazy_tensor.evaluate()
                    .sum(dim=-3)[:, : self.embedding_dim]
                )  # https://github.com/cornellius-gp/gpytorch/pull/871
            elif input.dim() == 2:
                input_1d = input.contiguous().view(1, -1)
                result = (
                    self.weight[input_1d[0]]
                    .base_lazy_tensor.evaluate()
                    .sum(dim=-3)[:, : self.embedding_dim]
                )  # TODO: Not sure if this selection (self.embedding_dim) is correct in here. # https://github.com/cornellius-gp/gpytorch/pull/871
